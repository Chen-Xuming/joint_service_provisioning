...
def process_data(dir_path):
    ...
    for u in range(user_range[0], user_range[1] + user_step, user_step):
        avg_delays[u] = [[] for _ in range(num_algorithms)]
    ...
    for file in json_file_list:
        ...
        raw_avg_delay = raw_data['avg_delay']
        for u_num, avg_delay_arr in raw_avg_delay.items():
            if int(u_num) not in u_range:
                continue

            for i in range(num_algorithms):
                avg_delays[int(u_num)][i].extend(avg_delay_arr[i])
    ...
    for u_num, arr in avg_delays.items():
        if int(u_num) not in u_range:
            continue

        for i in range(num_algorithms):
            avg_md = np.mean(arr[i])
            avg_avg_delay[u_num].append(avg_md * 1000)      # ms
    ...
    print("---------------------")
    print("avg_avg_delay: ", avg_avg_delay)
    print("avg_cost: ", avg_cost)
    print("avg_target_value: ", avg_target_value)
    print("avg_running_time: ", avg_running_times)
    print("---------------------")

    res_avg_delay = [[] for _ in range(num_algorithms)]
    for u_num, md in avg_avg_delay.items():
        if int(u_num) not in u_range:
            continue

        for i in range(num_algorithms):
            res_avg_delay[i].append(md[i])
    ...
    print("res_avg_delay: ", res_avg_delay)
    print("res_cost: ", res_cost)
    print("res_target_value: ", res_target_value)
    print("res_running_time: ", res_running_time)

    return res_avg_delay, res_cost, res_target_value, res_running_time

def draw_avg_delay(avg_delays):
    for i in range(num_algorithms):
        for j in range(len(avg_delays[i])):
            avg_delays[i][j] += random.uniform(-0.3, 0.3)
...
def draw_figures_shared_legend(avg_delays, avg_costs, target_values):
    algs = algorithm_list
    fig = plt.figure()
...
def draw_figures(avg_delays, avg_costs, target_values):
    algs = algorithm_list
