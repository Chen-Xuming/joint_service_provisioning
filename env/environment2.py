...
class Environment:
    def __init__(self, env_conf, env_seed):
        ...
        self.site_rng = default_rng(12345678)
    ...
    def init_site_attr(self):
        ...
        for site in self.areas[0].site_nodes:   # type: SiteNode
            site.service_rate_a = self.site_rng.integers(self.conf["minmax_service_rate_A_common"][0],
                                                        self.conf["minmax_service_rate_A_common"][1] + 1)
            site.service_rate_r = self.site_rng.integers(self.conf["minmax_service_rate_R_common"][0],
                                                        self.conf["minmax_service_rate_R_common"][1] + 1)
            site.tp_a = 1 / site.service_rate_a     # 秒
            site.tp_r = 1 / site.service_rate_r
            site.price_a = self.site_rng.integers(self.conf["minmax_price_A_common"][0], self.conf["minmax_price_A_common"][1] + 1)
            site.price_r = self.site_rng.integers(self.conf["minmax_price_R_common"][0], self.conf["minmax_price_R_common"][1] + 1)

        # 初始化 edge-site 的各个属性
        for area in self.areas[1:]:     # type: Area
            for site in area.site_nodes:
                site.service_rate_a = self.site_rng.integers(self.conf["minmax_service_rate_A_edge"][0],
                                                            self.conf["minmax_service_rate_A_edge"][1] + 1)
                site.service_rate_r = self.site_rng.integers(self.conf["minmax_service_rate_R_edge"][0],
                                                            self.conf["minmax_service_rate_R_edge"][1] + 1)
                site.tp_a = 1 / site.service_rate_a  # 秒
                site.tp_r = 1 / site.service_rate_r
                site.price_a = self.site_rng.integers(self.conf["minmax_price_A_edge"][0], self.conf["minmax_price_A_edge"][1] + 1)
                site.price_r = self.site_rng.integers(self.conf["minmax_price_R_edge"][0], self.conf["minmax_price_R_edge"][1] + 1)
    ...
    def show_site_info(self):
        for site in self.sites:     # type: SiteNode
            print("site #{}: [{}] service_rate: {}, {}   price: {}, {}".format(site.global_id,
                                                                               "Common" if site.area_id == 0 else "Edge",
                                                                               site.service_rate_a,
                                                                               site.service_rate_r,
                                                                               site.price_a,
                                                                               site.price_r))
