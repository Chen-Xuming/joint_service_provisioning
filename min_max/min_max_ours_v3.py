...
class MinMaxOurs_V3(BaseMinMaxAlgorithm):
    def __init__(self, env, consider_cost_tq=True, stable_only=False, *args, **kwargs):
        ...
        self.algorithm_name = "min_max_ours_v3" if "algorithm_name" not in kwargs else kwargs["algorithm_name"]
    ...
    def assign_users_c_one(self, c1: set, c1_pi: set):
        ...
        for c in c1:
            user = self.env.users[c]
            if user.service_a.node_id is not None:
                service_a_finished_flag[c] = True
            else:
                service_a_finished_flag[c] = False
                target_times += 1
        for c in c1_pi:
            user = self.env.users[c]
            if user.service_r.node_id is not None:
                service_r_finished_flag[c] = True
            else:
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        # assert abs(our_alg_v2.target_value - our_alg_v3.target_value) < 1e-6
