...
""" 创建文件夹 """
description = "min-avg_RA=True_SO=False"        # fixme
res_dir = "../result/min_avg/10-8_eta{}_{}".format(conf["eta"], description)
import os
...
algorithms = ["Nearest", "Modify-Assignment", "RA-UA"]
...
for num_user in range(user_range[0], user_range[1] + user_range_step, user_range_step):
    ...
    for i in range(simulation_times_each_num_user):
        ...
        for j in range(len(algorithms)):
            print("algorithm: {}, avg_delay = {}, cost = {}, target = {}, running_time = {}".format(algorithms[j], res_avg_delay[j][i],
                                                                                                    res_cost[j][i], res_target_value[j][i], res_running_time[j][i]))
        print("----------------------")
